...
from ultralytics.nn.modules import (AIFI, C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x,
                                    ...
                                    CoordAtt, CloFormer, ODConv_3rd, ConvNextBlock, HighPerfGAM, C2f_HighPerfGAM, C2f_HighPerfGAM_NoGlobal, C2f_HighPerfGAM_NoChannel, C2f_HighPerfGAM_NoLocal, ECAPlusPlus, ECAPlusPlus_MS, ECAPlusPlus_MS_TS, ECAPlusPlus_MS_TS_SC, C2f_Attention, Bottleneck_Attention, SimAM, EMA, CGA_2D)
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 ...
                 CoordAtt, ECA_Attention, CloFormer, ODConv_3rd, ConvNextBlock, HighPerfGAM, C2f_HighPerfGAM, C2f_HighPerfGAM_NoGlobal, C2f_HighPerfGAM_NoChannel, C2f_HighPerfGAM_NoLocal, ECAPlusPlus, ECAPlusPlus_MS, ECAPlusPlus_MS_TS, ECAPlusPlus_MS_TS_SC, C2f_Attention, Bottleneck_Attention, SimAM, EMA, CGA_2D):
            c1, c2 = ch[f], args[0]
        ...
        # """**************#add GAM Attention***************"""
            ...
            #c1, c2 = ch[f], args[0]
                ...
                #c2 = make_divisible(min(c2, max_channels) * width, 8)
            #args = [c1, *args[1:]]
        elif m is Shuffle_Attention:
            c1, c2 = ch[f], args[0]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, *args[1:]]
        elif m is EffectiveSE_Attention:
            c1, c2 = ch[f], args[0]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, *args[1:]]
        #elif m is ResBlock_CBAM_Attention:
            #c1, c2 = ch[f], args[0]
            #if c2 != nc:
                #c2 = make_divisible(min(c2, max_channels) * width, 8)
            #args = [c1, *args[1:]]
        elif m is Triplet_Attention:
            c1, c2 = ch[f], args[0]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, *args[1:]]
        #elif m is BasicRFB_Attention:
            #c1, c2 = ch[f], args[0]
            #if c2 != nc:
                #c2 = make_divisible(min(c2, max_channels) * width, 8)
            #args = [c1, *args[1:]]
