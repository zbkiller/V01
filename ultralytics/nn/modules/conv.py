...
class ECA_Attention(nn.Module):
    ...
    Args:
        ...
        gamma (float): Parameter for adaptive kernel size calculation. Default: 2.
        b (float): Parameter for adaptive kernel size calculation. Default: 1.
    """
...
class SimAM(nn.Module):
    ...
    Args:
        channels (int, optional): Number of input channels. Only for compatibility with YOLO parser. Default: None.
...
class ECAPlusPlus(nn.Module):
    ...
    def _build(self, channels):
        ...
        self.conv1 = nn.Conv1d(1, 1, kernel_size, padding=(kernel_size-1)//2, bias=False)
        kernel_size2 = max(3, kernel_size-2) if kernel_size > 5 else kernel_size
        self.conv2 = nn.Conv1d(1, 1, kernel_size2, padding=(kernel_size2-1)//2, bias=False)
    ...
    def forward(self, x):
        ...
        identity = x
        B, C, H, W = x.shape
        y = self.avg_pool(x)                     # 现在 avg_pool 始终存在
        ...
        y_att = y_scaled.transpose(-1, -2).unsqueeze(-1)
        channel_att = torch.sigmoid(y_att + self.interaction)
        spatial_context = F.adaptive_max_pool2d(x, 1)
