...
###################### EffectiveSE     ####     end   by  AI&CV  ###############################
####################################### CBAM、GAM、Resnet_CBAM https://cv2023.blog.csdn.net/article/details/129857671##########
...
class MultiHeadGAM(nn.Module):
    def __init__(self, channels, num_heads=8, reduction=16):
        super().__init__()
        ...
        self.spatial_att = nn.Sequential(
            ...
            *[MultiScaleConv(channels//4) for _ in range(3)],
            nn.Conv2d(channels//4, 1, 1),
            nn.Sigmoid()
        )
### Multi-Scale Feature Integration        
class MultiScaleGAM(nn.Module):
    def __init__(self, channels, scales=[1, 3, 5, 7]):
        ...
        self.fusion = nn.Conv2d(len(scales)*channels, channels, 1)
        
    def forward(self, x):
        scale_maps = [att(x) for att in self.scale_attentions]
        fused = torch.cat(scale_maps, dim=1)
        return x * self.fusion(fused)
 ### Position-Aware Attention
 class PositionAwareGAM(nn.Module):
    def __init__(self, channels):
        super().__init__()
        self.channels = channels
        
        # Learnable positional encoding
        self.pos_encoding = nn.Parameter(torch.randn(1, channels, 64, 64))
        
        # Relative position attention
        self.rel_pos_att = RelativePositionAttention(channels)
    ...
    def forward(self, x):
        ...
        att_map = self.rel_pos_att(x_pos)
        return x * att_map
 ### Lightweight Depthwise Attention
class LightweightGAM(nn.Module):
    def __init__(self, channels, reduction=4):
        super().__init__()
        
        # Depthwise channel attention (computationally efficient)
        self.channel_attention = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(channels, channels//reduction, 1, groups=channels//16),
            nn.GELU(),
            nn.Conv2d(channels//reduction, channels, 1, groups=channels//16),
            nn.Sigmoid()
        )
        ...
        # Depthwise fusion
        self.depthwise = nn.Conv2d(channels, channels, 3, 
                                   padding=1, groups=channels)
### Dynamic Kernel Attention
class DynamicKernelGAM(nn.Module):
    def __init__(self, channels):
        super().__init__()
        
        # Generate dynamic kernels based on input
        self.kernel_generator = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(channels, channels//4, 1),
            nn.ReLU(),
            nn.Conv2d(channels//4, 9, 1)  # 3×3 kernel parameters
        )
    ...
    def forward(self, x):
        ...
        kernel_params = self.kernel_generator(x).view(B, 1, 3, 3, H, W)
        
        # Apply dynamic convolution
        output = self.apply_dynamic_conv(x, kernel_params)
        return output
### Hierarchical Attention with Skip Connections
class HierarchicalGAM(nn.Module):
    def __init__(self, channels, levels=3):
        ...
        self.attention_layers = nn.ModuleList([
            SingleLevelAttention(channels // (2**i)) 
    ...
    def forward(self, x):
        ...
        for i in range(self.levels-1):
            features.append(self.downsamples[i](features[-1]))
        
        # Apply attention at each level
        att_features = []
        for i, feat in enumerate(features):
            att_feat = self.attention_layers[i](feat)
            att_features.append(att_feat)
        
        # Fuse back
        for i in range(self.levels-2, -1, -1):
            att_features[i] = att_features[i] + self.upsamples[i](att_features[i+1])
        
        return att_features[0]
### Channel-Spatial Decoupled Attention
class DecoupledGAM(nn.Module):
    def __init__(self, channels):
        super().__init__()
    ...
    def forward(self, x):
        channel_att = self.channel_path(x)
        spatial_att = self.spatial_path(x)
        
        # Adaptive combination
        combined_att = self.fusion_weight * channel_att + \
                      (1 - self.fusion_weight) * spatial_att
        
        return x * combined_att
### Transformer-Based Enhancement
class TransformerEnhancedGAM(nn.Module):
    def __init__(self, channels, num_heads=8, mlp_ratio=4):
        super().__init__()
        
        # Local attention (CNN-based)
        ...
        # Global attention (Transformer-based)
    ...
    def forward(self, x):
        ...
        # Global attention
        att_output, _ = self.global_att(x_flat, x_flat, x_flat)
        att_output = self.norm1(att_output + x_flat)
        
        # MLP
        mlp_output = self.mlp(att_output)
        global_feat = self.norm2(mlp_output + att_output)
...
### Enhanced GAM for YOLOv8 (Balanced)
